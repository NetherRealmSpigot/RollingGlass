...
pub async fn ping(host: &String, port: &u16, fakehost: &String, protocol: &ProtocolNum, timeout: &u8) -> Result<Vec<u8>, String> {
    ...
    if fakehost.is_empty() {
        host_touse = String::from(host);
    } else {
        host_touse = String::from(fakehost);
    }

    let port_touse;
    if *port == 0 {
        port_touse = DEFAULT_PORT;
    } else {
        port_touse = *port;
    }

    let ips = resolve(&*host, &port_touse).await?;
    ...
    for v in ips.iter() {
        ...
        let resize = read_varint(&mut stream, &mut slice::from_mut(&mut byte));
        if !resize.is_ok() {
            return Err(format!("{}", resize.err().unwrap()).to_string());
        }

        stream.read(&mut slice::from_mut(&mut byte)).expect("Cannot read packet ID");
        if byte != 0u8 {
            shutoff(&mut stream);
        ...
        while bytes_read < resize {
            ...
            if !read_res.is_ok() {
                shutoff(&mut stream);
                return Err(format!("{}", read_res.err().unwrap()).to_string());
            }
...
fn read_varint(stream: &mut TcpStream, buf: &mut[u8]) -> Result<usize, &'static str> {
    let mut res = 0i32;
    for i in 0.. {
        if i > 5 {
            shutoff(stream);
        ...
        res = res | (((buf[0] as i32) & 0x7Fi32) << (7 * i));
